...
;; There are very few basic Ushell commands -- so-called builtins.
...
(defcustom ushell-plain-echo-behavior nil
...
;;   echo Hello world
;;
;; If `ushell-plain-echo-behavior' is non-nil, this will yield the
...
;;;_* `version'
;;
;; This command reports the version number for Ushell and all its
...
(defun ushell-echo (args &optional output-newline)
  "Implementation code for a Lisp version of `echo'.
It returns a formatted value that should be passed to `ushell-print'
or `ushell-printn' for display."
  (if ushell-plain-echo-behavior
      (concat (apply 'ushell-flatten-and-stringify args) "\n")
    (let ((value
	   (cond
	    ((= (length args) 0) "")
	    ((= (length args) 1)
	  ...
	  (cond
	   ...
	   ((listp value)
	    (append value (list "\n")))
	   (t
	    (concat (ushell-stringify value) "\n")))
	value))))

(defun ushell/echo (&rest args)
  "Implementation of `echo'.  See `ushell-plain-echo-behavior'."
  (ushell-eval-using-options
   ...
   '((?n nil nil output-newline "terminate with a newline")
     ...
     :preserve-args
     :usage "[-n] [object]")
   (ushell-echo args output-newline)))

(defun ushell/printnl (&rest args)
  "Print out each of the argument, separated by newlines."
  (let ((elems (ushell-flatten-list args)))
    (while elems
      (ushell-printn (ushell-echo (list (car elems))))
      (setq elems (cdr elems)))))

(defun ushell/listify (&rest args)
...
(defun ushell/umask (&rest args)
  ...
  (ushell-eval-using-options
   ...
   (if (or (not args) symbolic-p)
	 ...
	 (ushell-printn modstr))
     (setcar args (ushell-convert (car args)))
     (if (numberp (car args))
