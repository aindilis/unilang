...
(defcustom ushell-pred-load-hook '(ushell-pred-initialize)
  "*A list of functions to run when `ushell-pred' is loaded."
  :type 'hook
  :group 'ushell-pred)

(defcustom ushell-predicate-alist
  '((?/ . (ushell-pred-file-type ?d))   ; directories
    (?. . (ushell-pred-file-type ?-))   ; regular files
    (?s . (ushell-pred-file-type ?s))   ; sockets
    (?p . (ushell-pred-file-type ?p))   ; named pipes
    (?@ . (ushell-pred-file-type ?l))   ; symbolic links
    (?% . (ushell-pred-file-type ?%))   ; allow user to specify (c def.)
    (?r . (ushell-pred-file-mode 0400)) ; owner-readable
    (?w . (ushell-pred-file-mode 0200)) ; owner-writable
    (?x . (ushell-pred-file-mode 0100)) ; owner-executable
    (?A . (ushell-pred-file-mode 0040)) ; group-readable
    (?I . (ushell-pred-file-mode 0020)) ; group-writable
    (?E . (ushell-pred-file-mode 0010)) ; group-executable
    (?R . (ushell-pred-file-mode 0004)) ; world-readable
    (?W . (ushell-pred-file-mode 0002)) ; world-writable
    (?X . (ushell-pred-file-mode 0001)) ; world-executable
    (?s . (ushell-pred-file-mode 4000)) ; setuid
    (?S . (ushell-pred-file-mode 2000)) ; setgid
    (?t . (ushell-pred-file-mode 1000)) ; sticky bit
...
;;;              (= (nth 2 (file-attributes file)) (user-uid)))))
    ...
    (?l . (ushell-pred-file-links))
    (?u . (ushell-pred-user-or-group ?u "user" 2 'ushell-user-id))
    (?g . (ushell-pred-user-or-group ?g "group" 3 'ushell-group-id))
    (?a . (ushell-pred-file-time ?a "access" 4))
    (?m . (ushell-pred-file-time ?m "modification" 5))
    (?c . (ushell-pred-file-time ?c "change" 6))
    (?L . (ushell-pred-file-size)))
...
(put 'ushell-predicate-alist 'risky-local-variable t)

(defcustom ushell-modifier-alist
  '((?e . '(lambda (lst)
	     (mapcar
	      (function
	       (lambda (str)
		 (ushell-stringify
		  (car (ushell-parse-argument str))))) lst)))
    (?L . '(lambda (lst)
	     (mapcar 'downcase lst)))
    (?U . '(lambda (lst)
	     (mapcar 'upcase lst)))
    ...
    (?i . (ushell-include-members))
    (?x . (ushell-include-members t))
    (?r . '(lambda (lst)
    ...
    (?q . '(lambda (lst)
	     (mapcar 'ushell-escape-arg lst)))
    (?u . '(lambda (lst)
	     (ushell-uniqify-list lst)))
    (?o . '(lambda (lst)
	     (sort lst 'string-lessp)))
    (?O . '(lambda (lst)
	     (nreverse (sort lst 'string-lessp))))
    (?j . (ushell-join-members))
    (?S . (ushell-split-members))
    (?R . 'reverse)
    (?g . (progn
	    (forward-char)
	    (if (eq (char-before) ?s)
		(ushell-pred-substitute t)
	      (error "`g' modifier cannot be used alone"))))
    (?s . (ushell-pred-substitute)))
...
(put 'ushell-modifier-alist 'risky-local-variable t)

(defvar ushell-predicate-help-string
  "Ushell predicate quick reference:
...
(defvar ushell-modifier-help-string
  "Ushell modifier quick reference:
...
(defun ushell-display-predicate-help ()
  (interactive)
  (with-electric-help
   (function
    (lambda ()
      (insert ushell-predicate-help-string)))))

(defun ushell-display-modifier-help ()
  (interactive)
  (with-electric-help
   (function
    (lambda ()
      (insert ushell-modifier-help-string)))))

(defun ushell-pred-initialize ()
  "Initialize the predicate/modifier code."
  (make-local-hook 'ushell-parse-argument-hook)
  (add-hook 'ushell-parse-argument-hook
	    'ushell-parse-arg-modifier t t)
  (define-key ushell-command-map [(meta ?q)] 'ushell-display-predicate-help)
  (define-key ushell-command-map [(meta ?m)] 'ushell-display-modifier-help))

(defun ushell-apply-modifiers (lst predicates modifiers)
  ...
  (let (stringified)
	...
	(setq lst (list lst)
	      stringified t))
    (when (listp lst)
      (setq lst (ushell-winnow-list lst nil predicates))
      (while modifiers
...
(defun ushell-parse-arg-modifier ()
  "Parse a modifier that has been specified after an argument.
This function is specially for adding onto `ushell-parse-argument-hook'."
  (when (eq (char-after) ?\()
    (forward-char)
	...
	(when (ushell-arg-delimiter (1+ end))
	  (save-restriction
	    ...
	    (let* ((modifiers (ushell-parse-modifiers))
		   (preds (car modifiers))
		   (mods (cdr modifiers)))
	      (if (or preds mods)
		  ...
		  (setq ushell-current-modifiers
			(append
			 ...
			 (list
			  `(lambda (lst)
			     (ushell-apply-modifiers
			      lst (quote ,preds) (quote ,mods)))))))))
	  (goto-char (1+ end))
	  (ushell-finish-arg))))))

(defun ushell-parse-modifiers ()
	    ...
	    (cond
	     ((eq char ?')
		  ...
		  (let ((func (read (current-buffer))))
			...
			(setq preds (ushell-add-pred-func func preds
							  negate follow))
		      (error "Invalid function predicate '%s'"
			     (ushell-stringify func))))
...
(defun ushell-pred-user-or-group (mod-char mod-type attr-index get-id-func)
  ...
  (let (ugid open close end)
	...
	(setq close open))
      (forward-char)
      (setq end (ushell-find-delimiter open close))
      (unless end
...
(defun ushell-pred-file-time (mod-char mod-type attr-index)
  ...
  (let* ((quantum 86400)
    ...
    (if (looking-at "[0-9]+")
	(progn
	  (setq when (- (ushell-time-to-seconds (current-time))
	...
	(setq when (ushell-time-to-seconds (nth attr-index attrs))))
      (goto-char (1+ end)))
    `(lambda (file)
       (let ((attrs (file-attributes file)))
	 (if attrs
		 ...
		 (if (eq qual ?+)
		     '>
		   '=)) ,when (ushell-time-to-seconds
			       (nth ,attr-index attrs))))))))

(defun ushell-pred-file-type (type)
...
that 'ls -l' will show in the first column of its display. "
  (when (eq type ?%)
	...
	(forward-char)
      (setq type ?%)))
  `(lambda (file)
     (let ((attrs (ushell-file-attributes (directory-file-name file))))
       (if attrs
...
(defsubst ushell-pred-file-mode (mode)
...
(defun ushell-pred-file-links ()
  ...
  (let (qual amount)
    ...
    (goto-char (match-end 0))
    `(lambda (file)
       (let ((attrs (ushell-file-attributes file)))
	 (if attrs
	     (,(if (eq qual ?-)
		   '<
		 (if (eq qual ?+)
...
(defun ushell-pred-substitute (&optional repeat)
  "Return a modifier function that will substitute matches."
  (let ((delim (char-after))
	match replace end)
    (forward-char)
    (setq end (ushell-find-delimiter delim delim nil nil t)
...
(defun ushell-include-members (&optional invert-p)
...
(defun ushell-split-members ()
  "Return a modifier function that splits members."
  (let ((delim (char-after))
	sep end)
    (when (memq delim '(?' ?/))
      (forward-char)
      (setq end (usell-find-delimiter delim delim nil nil t)
