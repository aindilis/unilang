;;; ush-opt.el --- command options processing
...
(defmacro ushell-eval-using-options (name macro-args
...
`ushell-eval-using-options' returns the value of the last form in
BODY-FORMS.  If instead an external command is run, the tag
`ushell-external' will be thrown with the new process for its value.
...
interned variable `args' (created using a `let' form)."
  `(let ((temp-args
	  ,(if (memq ':preserve-args (cadr options))
	       macro-args
	     (list 'ushell-stringify-list
		   (list 'ushell-flatten-list macro-args)))))
     (let ,(append (mapcar (function
       ...
       (ushell-do-opt ,name ,options (quote ,body-forms)))))
...
(defun ushell-do-opt (name options body-forms)
  "Helper function for `ushell-eval-using-options'.
This code doesn't really need to be macro expanded everywhere."
  (setq args temp-args)
  (if (setq
       ext-command
       (catch 'ushell-ext-command
	 (when (setq
		usage-msg
		(catch 'ushell-usage
		  (setq last-value nil)
		  (if (and (= (length args) 0)
		      ...
		      (throw 'ushell-usage
			     (ushell-show-usage name options)))
		  (setq args (ushell-process-args name args options)
			last-value (eval (append (list 'progn)
						 body-forms)))
		  nil))
	   (error usage-msg))))
      (throw 'ushell-external
	     (ushell-external-command ext-command args))
    last-value))

(defun ushell-show-usage (name options)
  ...
  (let ((usage (format "usage: %s %s\n\n" name
	...
	(setq usage (concat usage (and had-option "\n")
			    (cadr post-usage))))
    (when extcmd
      (setq extcmd (ushell-search-path (cadr extcmd)))
      (if extcmd
	  (setq usage
		(concat usage
			(format "
...
will be called instead." extcmd)))))
    (throw 'ushell-usage usage)))

(defun ushell-set-option (name ai opt options)
...
(defun ushell-process-option (name switch kind ai options)
...
switch is unrecognized."
  (let* ((opts options)
    ...
    (while opts
	  ...
	  (progn
	    (ushell-set-option name ai (car opts) options)
	...
	(when extcmd
	  (setq extcmd (ushell-search-path (cadr extcmd)))
	  (if extcmd
	      (throw 'ushell-ext-command extcmd)
	    (if (char-valid-p switch)
		(error "%s: unrecognized option -%c" name switch)
	      (error "%s: unrecognized option --%s" name switch))))))))

(defun ushell-process-args (name args options)
  "Process the given ARGS using OPTIONS.
This assumes that symbols have been intern'd by `ushell-with-options'."
  (let ((ai 0) arg)
	...
	(let* ((dash (match-string 1 arg))
	  ...
	  (if dash
	      (if (> (length switch) 0)
		  (ushell-process-option name switch 1 ai options)
		(setq ai (length args)))
	    (let ((len (length switch))
		  (index 0))
	      (while (< index len)
		(ushell-process-option name (aref switch index) 0 ai options)
		(setq index (1+ index)))))))))
  args)
