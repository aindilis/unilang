;;; ush-var.el --- handling of variables
...
;;   ${command}
;;
;; Returns the value of an ushell subcommand.  See the note above
...
;; equal to "hello".
;;
;; There are also a few special variables defined by Ushell.  '$$' is
...
(defcustom ushell-var-load-hook '(ushell-var-initialize)
  "*A list of functions to call when loading `ushell-var'."
  :type 'hook
  :group 'ushell-var)

(defcustom ushell-prefer-lisp-variables nil
  "*If non-nil, prefer Lisp variables to environment variables."
  :type 'boolean
  :group 'ushell-var)

(defcustom ushell-complete-export-definition t
  "*If non-nil, completing names for `export' shows current definition."
  :type 'boolean
  :group 'ushell-var)

(defcustom ushell-variable-name-regexp "[A-Za-z0-9_-]+"
...
(defcustom ushell-variable-aliases-list
  '(;; for ushell.el
    ...
    ("_" (lambda (indices)
	   (if (not indices)
	       (car (last ushell-last-arguments))
	     (ushell-apply-indices ushell-last-arguments
				   indices))))
    ("?" ushell-last-command-status)
    ("$" ushell-last-command-result)
    ("0" ushell-command-name)
    ("1" (lambda (indices) (nth 0 ushell-command-arguments)))
    ("2" (lambda (indices) (nth 1 ushell-command-arguments)))
    ("3" (lambda (indices) (nth 2 ushell-command-arguments)))
    ("4" (lambda (indices) (nth 3 ushell-command-arguments)))
    ("5" (lambda (indices) (nth 4 ushell-command-arguments)))
    ("6" (lambda (indices) (nth 5 ushell-command-arguments)))
    ("7" (lambda (indices) (nth 6 ushell-command-arguments)))
    ("8" (lambda (indices) (nth 7 ushell-command-arguments)))
    ("9" (lambda (indices) (nth 8 ushell-command-arguments)))
    ("*" (lambda (indices)
	   (if (not indices)
	       ushell-command-arguments
	     (ushell-apply-indices ushell-command-arguments
				   indices)))))
  "*This list provides aliasing for variable references.
It is very similar in concept to what `ushell-user-aliases-list' does
...
'(10 20)', and nil."
  :type '(repeat (list string sexp
		       (choice (const :tag "Copy to environment" t)
			       (const :tag "Use only in Ushell" nil))))
  :group 'ushell-var)

(put 'ushell-variable-aliases-list 'risky-local-variable t)

;;; Functions:

(defun ushell-var-initialize ()
  "Initialize the variable handle code."
  ;; Break the association with our parent's environment.  Otherwise,
  ;; changing a variable will affect all of Emacs.
  (set (make-local-variable 'process-environment) (ushell-copy-environment))

  (define-key ushell-command-map [(meta ?v)] 'ushell-insert-envvar)

  (set (make-local-variable 'ushell-special-chars-inside-quoting)
       (append ushell-special-chars-inside-quoting '(?$)))
  (set (make-local-variable 'ushell-special-chars-outside-quoting)
       (append ushell-special-chars-outside-quoting '(?$)))

  (make-local-hook 'ushell-parse-argument-hook)
  (add-hook 'ushell-parse-argument-hook 'ushell-interpolate-variable t t)

  (make-local-hook 'ushell-prepare-command-hook)
  (add-hook 'ushell-prepare-command-hook
	    'ushell-handle-local-variables nil t)

  (when (ushell-using-module 'ushell-cmpl)
    (make-local-hook 'pcomplete-try-first-hook)
    (add-hook 'pcomplete-try-first-hook
	      'ushell-complete-variable-reference nil t)
    (add-hook 'pcomplete-try-first-hook
	      'ushell-complete-variable-assignment nil t)))

(defun ushell-handle-local-variables ()
  ...
  ;; interpolation pass before the argument parsing pass, but Ushell
  ;; does both at the same time).
  (while (and (not ushell-last-command-name)
	      ushell-last-arguments)
    (setq ushell-last-command-name (car ushell-last-arguments)
	  ushell-last-arguments (cdr ushell-last-arguments)))
  (let ((setvar "\\`\\([A-Za-z_][A-Za-z0-9_]*\\)=\\(.*\\)\\'")
	(command (ushell-stringify ushell-last-command-name))
	(args ushell-last-arguments))
	...
	(throw
	 'ushell-replace-command
	 (list
	  'ushell-as-subcommand
	  (append
	   (list 'progn)
	   (let ((l (list t)))
		...
		l (list
	       ...
	       (setq command (ushell-stringify (car args))
		     args (cdr args)))
	     (cdr l))
	   (list (list 'ushell-named-command
		       command (list 'quote args)))))))))

(defun ushell-interpolate-variable ()
  "Parse a variable interpolation.
This function is explicit for adding to `ushell-parse-argument-hook'."
  (when (and (eq (char-after) ?$)
	     (/= (1+ (point)) (point-max)))
    (forward-char)
    (list 'ushell-escape-arg
	  (ushell-parse-variable))))

(defun ushell/define (var-alias definition)
  "Define a VAR-ALIAS using DEFINITION."
  (if (not definition)
      (setq ushell-variable-aliases-list
	    (delq (assoc var-alias ushell-variable-aliases-list)
		  ushell-variable-aliases-list))
    (let ((def (assoc var-alias ushell-variable-aliases-list))
	  (alias-def
	   (list var-alias
...
(defun pcomplete/ushell-mode/export ()
  "Completion function for Ushell's `export'."
  (while (pcomplete-here
	  (if ushell-complete-export-definition
	      process-environment
	    (ushell-envvar-names)))))

(defun ushell/unset (&rest args)
...
(defun pcomplete/ushell-mode/unset ()
  "Completion function for Ushell's `unset'."
  (while (pcomplete-here (ushell-envvar-names))))

(defun ushell/setq (&rest args)
...
(defun pcomplete/ushell-mode/setq ()
  "Completion function for Ushell's `setq'."
  (while (and (pcomplete-here (all-completions pcomplete-stub
					       obarray 'boundp))
	      (pcomplete-here))))

(defun ushell/env (&rest args)
  ...
  (ushell-eval-using-options
   ...
   '((?h "help" nil nil "show this usage screen")
     :external "env"
     :usage "<no arguments>")
   (ushell-for setting (sort (ushell-environment-variables)
			     'string-lessp)
     (ushell-buffered-print setting "\n"))
   (ushell-flush)))

(defun ushell-insert-envvar (envvar-name)
...
(defun ushell-envvar-names (&optional environment)
...
(defun ushell-environment-variables ()
  "Return a `process-environment', fully updated.
This involves setting any variable aliases which affect the
environment, as specified in `ushell-variable-aliases-list'."
  (let ((process-environment (ushell-copy-environment)))
    (ushell-for var-alias ushell-variable-aliases-list
      (if (nth 2 var-alias)
	  (setenv (car var-alias)
		  (ushell-stringify
		   (or (ushell-get-variable (car var-alias)) "")))))
    process-environment))

(defun ushell-parse-variable ()
...
`ushell-prefer-lisp-variables'.

Its purpose is to call `ushell-parse-variable-ref', and then to
process any indices that come after the variable reference."
  (let* ((get-len (when (eq (char-after) ?#)
		    (forward-char) t))
	 value indices)
    (setq value (ushell-parse-variable-ref)
	  indices (and (not (eobp))
		       (eq (char-after) ?\[)
		       (ushell-parse-indices))
	  value (list 'let
		      (list (list 'indices
				  (list 'quote indices)))
		      value))
...
(defun ushell-parse-variable-ref ()
...
Possible options are:
  ...
  (let (end)
	...
	(if name
	  (list 'ushell-get-variable (eval name) 'indices))))
     ((eq (char-after) ?<)
      (let ((end (ushell-find-delimiter ?\< ?\>)))
	(if (not end)
	    (throw 'ushell-incomplete ?\<)
	  (let* ((temp (make-temp-name temporary-file-directory))
	    ...
	    (prog1
		(list
		 'let (list (list 'ushell-current-handles
				  (list 'ushell-create-handles temp
					(list 'quote 'overwrite))))
		 (list
		  'progn
		  (list 'ushell-as-subcommand
			(ushell-parse-command cmd))
		  (list 'ignore
			(list 'nconc 'ushell-this-command-hook
			      (list 'list
				    (list 'function
					  (list 'lambda nil
	...
	(end-of-file
     ...
     ((assoc (char-to-string (char-after))
	     ushell-variable-aliases-list)
      (forward-char)
      (list 'ushell-get-variable
	    (char-to-string (char-before)) 'indices))
     ((looking-at ushell-variable-name-regexp)
      (prog1
	  (list 'ushell-get-variable (match-string 0) 'indices)
	(goto-char (match-end 0))))
     (t
      (error "Invalid variable reference")))))

(ushell-deftest var interp-cmd
  "Interpolate command result"
  (ushell-command-result-p "+ ${+ 1 2} 3" "6\n"))

(ushell-deftest var interp-lisp
  "Interpolate Lisp form evalution"
  (ushell-command-result-p "+ $(+ 1 2) 3" "6\n"))

(ushell-deftest var interp-concat
  "Interpolate and concat command"
  (ushell-command-result-p "+ ${+ 1 2}3 3" "36\n"))

(ushell-deftest var interp-concat-lisp
  "Interpolate and concat Lisp form"
  (ushell-command-result-p "+ $(+ 1 2)3 3" "36\n"))

(ushell-deftest var interp-concat2
  "Interpolate and concat two commands"
  (ushell-command-result-p "+ ${+ 1 2}${+ 1 2} 3" "36\n"))

(ushell-deftest var interp-concat-lisp2
  "Interpolate and concat two Lisp forms"
  (ushell-command-result-p "+ $(+ 1 2)$(+ 1 2) 3" "36\n"))

(defun ushell-parse-indices ()
  ...
  (let (indices)
	...
	(if (not end)
	    (throw 'ushell-incomplete ?\[)
	  (forward-char)
	  (let (ushell-glob-function)
	    (setq indices (cons (ushell-parse-arguments (point) end)
				indices)))
	  (goto-char (1+ end)))))
    (nreverse indices)))

(defun ushell-get-variable (name &optional indices)
  "Get the value for the variable NAME."
  (let* ((alias (assoc name ushell-variable-aliases-list))
	 (var (if alias
		  (cadr alias)
		name)))
    (if (and alias (functionp var))
	(funcall var indices)
      (ushell-apply-indices
       (cond
	((stringp var)
	 (let ((sym (intern-soft var)))
	   (if (and sym (boundp sym)
		    (or ushell-prefer-lisp-variables
			(not (getenv var))))
	       (symbol-value sym)
	     (getenv var))))
	((symbolp var)
	 (symbol-value var))
	(t
	 (error "Unknown variable `%s'" (ushell-stringify var))))
       indices))))

(defun ushell-apply-indices (value indices)
  ...
  (while indices
	...
	(let (separator)
	  (if (not (or (not (stringp (caar indices)))
		       (string-match
			(concat "^" ushell-variable-name-regexp "$")
			(caar indices))))
	      (setq separator (caar indices)
		    refs (cdr refs)))
	  (setq value
		(mapcar 'ushell-convert
			(split-string value separator)))))
      (cond
       ((< (length refs) 0)
	(error "Illegal array variable index: %s"
	       (ushell-stringify refs)))
       ((= (length refs) 1)
	(setq value (ushell-index-value value (car refs))))
       (t
	(let ((new-value (list t)))
	  (while refs
	    (nconc new-value
		   (list (ushell-index-value value
					     (car refs))))
	    (setq refs (cdr refs)))
...
(defun ushell-index-value (value index)
...
(defun ushell-complete-variable-reference ()
  ...
  (let ((arg (pcomplete-actual-arg)) index)
    (when (setq index
      ...
      (throw 'pcomplete-completions (ushell-variables-list)))))

(defun ushell-variables-list ()
  "Generate list of applicable variables."
  (let ((argname pcomplete-stub)
	completions)
    (ushell-for alias ushell-variable-aliases-list
    ...
    (sort
     (append
      (mapcar
       (function
	(lambda (varname)
	  (let ((value (ushell-get-variable varname)))
	    (if (and value
		     (stringp value)
		     (file-directory-p value))
		(concat varname (char-to-string directory-sep-char))
	      varname))))
       (ushell-envvar-names (ushell-environment-variables)))
      (all-completions argname obarray 'boundp)
      completions)
     'string-lessp)))

(defun ushell-complete-variable-assignment ()
  "If there is a variable assignment, allow completion of entries."
  (let ((arg (pcomplete-actual-arg)) pos)
    (when (string-match (concat "\\`" ushell-variable-name-regexp "=") arg)
