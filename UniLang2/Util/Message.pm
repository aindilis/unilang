...
use Class::MethodMaker
  new_with_init => 'new',
  get_set       => [ qw / ID Sender Receiver Date Contents Data MyXMLDumper DataFormat / ];
...
sub init {
  ...
  if ($args{Message}) {
    my $m = $args{Message};
    if (ref $m =~ /UniLang2::Util::Message/) { # should really have
                                              # something that can
