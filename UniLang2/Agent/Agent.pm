...
use Class::MethodMaker
  ...
  [
   ...
   qw / Name Client ReceiveHandler STDINHandler Buffer BufferMessage
	...
	MyINotify Name MessageDir IncomingDir ServerIncomingDir
	MessageCounter /
...
sub init {
  ...
  MkDirIfNotExists
    (
     Directory => dirname($self->IncomingDir),
    );
  $self->ServerIncomingDir(ConcatDir($self->MessageDir,"server","9000","incoming"));
