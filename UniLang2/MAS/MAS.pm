package UniLang2::MAS::MAS;

use UniLang2::MAS::Server;
use UniLang2::MAS::Agent;
use UniLang2::Util::Log;
...
{
  my $server;
  my $log = UniLang2::Util::Log->new;

  sub init {
    my ($self,%args) = @_;
    $self->Host($args{Host} || "localhost");
    $self->Port($args{Port} || "9000");
    $server = UniLang2::MAS::Server->new
      (
       MAS => $self,
       Host => $self->Host,
       Port => $self->Port,
...
sub AddAgent {
  my ($self,%args) = @_;
  $self->Agents->{$args{Agent}->Name} = $args{Agent};
...
sub RemoveAgent {
  my ($self,%args) = @_;
  delete $self->Agents->{$args{Agent}->Name};
...
sub IsaAgent {
  ...
  #   my $newmessage = UniLang2::Util::Message->new
  #     (
  #      Sender => "UniLang",
...
sub ProcessCommunicationEvent {
  ...
  if ($client->Agent->Name eq $client->Name) {
	...
	(Message => $message,
	 ClientName => $agent->Client->Name);
      if ($self->Server->IsConnected(ClientName => $agent->Client->Name)) {
  ...
  if ($client->Agent->Name ne $sender) {
    ...
    if ($receiver eq "UniLang") {
      # if addressed to UniLang2, call a special function
...
sub AnalyzeContents {
  ...
  if ($contents =~ /^UniLang,\s*(.*)$/i) {
    ...
    my $message2 = UniLang2::Util::Message->new
      (Message => $message);
...
sub SendMessageWithAutoVivification {
  my ($self, %args) = @_;
  print Dumper({SendMessageWithAutoVivification => \%args}) if 0;
  my $name = $args{Name};
  my $message = $args{Message};
...
sub DeRegister {
  ...
  if ($self->IsaAgent(Name => $args{Agent})) {
    ...
    $self->Server->RemoveClient(ClientName => $agent->Client->Name);
...
sub ProcessArbitraryMessage {
  ...
  # for now just write it to unilang2 logfile as normal
