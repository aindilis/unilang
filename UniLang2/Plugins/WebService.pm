package UniLang2::Plugins::WebService;

# I guess the model is, if you wish to connect to the remote unilang2,
# you connect to your local unilang2 first...  how will that work with
# people who don't have unilang2?  We'll need a client API for them
...
sub init {
  ...
  UniLang2::Plugin->init(%args);
  $self->MyTempAgent
    (UniLang2::Util::TempAgent->new
     (
      Name => $self->Name,
      Properties => {
...
sub WebServiceReceive {
  my ($self,%args) = @_;
  # if we have received a message from a remote unilang2 instance
  # through the webservice, call this function with the message, it
  # will query our unilang2, and return the answer
...
sub UniLangReceive {
  ...
  my $message = $args{Message};
  print Dumper({"UniLang2::Plugins::WebService::UniLangReceive" => $message});
  $self->ProcessMessage
    (
...
sub ListLocallyAvailableAgents {
  ...
  # to the connecting client, asking it what it's unilang2 agents are,
  # and then return that information

  # we have to query the Local UniLang2 for this information
}
...
sub ListRemotelyAvailableAgents {
  ...
  # we have to send a webservice message querying the remote UniLang2
}
